...
mod tests {
    ...
    #[test]
    fn test_apply_op_inverse_sum_to_zero() {
        let chans = 2;
        let frames = 64;
        let initial_val = 10.0;

        let mut node = ApplyOp::new(initial_val, chans, add);
        let mut ctx = AudioContext::default();

        let mut in_0 = vec![5.0; frames];
        let mut in_1 = vec![-5.0; frames];

        let mut out_0 = vec![0.0; frames];
        let mut out_1 = vec![0.0; frames];
        ...
        // 4. Assertions
        // Channel 0 should be 5.0 + 10.0 = 15.0
        // Channel 1 should be -5.0 + 10.0 = 5.0
        // The sum of (Out0 + Out1) should be 2 * val (20.0)
        // But specifically, the difference between them should match the original signal difference
        for i in 0..frames {
            assert_eq!(out_0[i], 15.0, "Channel 0 calculation mismatch");
            assert_eq!(out_1[i], 5.0, "Channel 1 calculation mismatch");

            // To test the "inverse" logic: (In + Val) + ((-In) + Val) = 2 * Val
            assert_eq!(out_0[i] + out_1[i], 20.0);
        }
    }
}
