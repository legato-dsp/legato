...
/// All of the interior delay lengths below are expressed in samples at the
/// original design's reference rate and rescaled to the actual sample rate.
...
/// Inspired by the Lexicon 480L: a Dattorro-style plate with a figure-eight
/// feedback tank, which is why it is written per-sample.
///
...
impl Plate480 {
    ...
    pub fn from_params(
        ...
        Ok(Self::new(sr, predelay, decay, damping, bandwidth, mix))
    }
}
...
impl NodeDefinition for Plate480 {
    ...
    fn create(
        ...
        // The tank is inherently per-sample (figure-eight feedback), so the
        // block-rate node is just the PerSample adaptation of the same DSP.
        Ok(Box::new(PerSample::new(Self::from_params(rb, p)?)))
    }
}
