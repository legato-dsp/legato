...
impl NodeDefinition for Svf {
    ...
    const DESCRIPTION: &'static str = "State variable filter (lowpass, highpass, bandpass, and more)";
    ...
    fn create(rb: &mut ResourceBuilderView, p: &DSLParams) -> Result<Box<dyn DynNode>, ValidationError> {
        ...
        let filter_type = p.get_str("type").map_or(FilterType::LowPass, |f| match f.as_str() {
