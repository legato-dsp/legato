...
Nodes in Legato are individual units responsible for audio processing.
You can extend the included nodes at the start of the application runtime. 
This allows users the ability to easily modify the framework to meet their usecase.

### When should you use a custom node?

Basically, if you need some external logic, custom DSP integration, sub-block size feedback loops, interaction with system resources, etc. and this must happen at audio rate (per sample information), you should look into a custom node.
...
Additionally, there are helper functions to write nodes that can process per_sample and be adapted to block rate.
This is quite helpful for FM with feedback, FDN, etc.

For a kitchen sink custom node example, checkout `examples/custom_lexicon.rs`.